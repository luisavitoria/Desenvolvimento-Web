...
<body>
    ...
    if (isset($_POST['save'])) {
        ...
        if (!empty($name) && !empty($phone) && !empty($age) && !empty($local)) {
            $sql = "SELECT * FROM moradia WHERE idmoradia='$id'";
            $result = $conn->query($sql);

            if ($result->num_rows > 0) {
                $sql = "UPDATE moradia SET nome='$name', telefone='$phone', idade='$age', local='$local' WHERE idmoradia='$id'";
    ...
    <form method="post" action="">
        ...
        <button type="submit" class="button-save" name="save">Alterar</button>
    </form>
</body>
